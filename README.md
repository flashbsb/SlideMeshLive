...
16. **Security Architecture, Multi-Auth Governance Matrix & Setup Wizard**:
    - **Multi-Auth Governance Matrix**: Unified and configurable support across 3 authentication mechanisms (**🔑 Master PIN / Deck PIN**, **👤 Local RBAC Accounts with Admin/Presenter roles**, and **🌐 Google Workspace SSO Whitelist**), enforcing contextual protection across all 5 core interfaces: Control Room (`/admin/`), Stage Screen (`/presenter/`), Main Portal (`/index.html`), SlideMesh Studio (`/import.html`), and Mobile Audience (`/audience/`).
    - **Credential Protection & Server-Side Gatekeeper**: Direct HTTP requests to `config/security.json` are blocked with `403 Forbidden` and `"pin"` keys are dynamically redacted from public manifests, delegating 100% of PIN/password validation to the backend (`POST /api/auth/verify-pin` and `/api/auth/login`).
    - **First-Run Security Setup Wizard (Web & CLI)**: When launched without credentials, users are guided via `setup.html` (with interactive method selection, protection scope configuration, and password strength meter) or via interactive CLI `python3 server.py --setup` to provision security infrastructure with atomic writes (`POST /api/auth/setup`).
    - **Control Room Security & Matrix Management Panel**: Full governance modal accessible via **`🔐 Security`** button in the Admin header with 4 interactive tabs (**🔑 Multi-Auth Matrix & Scopes**, **👥 Local Users & Speakers**, **📱 Offline Audience**, and **🌐 Google Workspace Whitelist**) featuring anti-lockout protection and instant runtime sync via `POST /api/security/config` without restarting Python.
    - **Network Hardening & Anti-Spoofing Protection**: Strict rejection of unauthorized administrative `/api/sync` actions (`403 Forbidden`), HTML sanitization against XSS in Q&A/votes, anti-flooding rate-limiting (`429 Too Many Requests`), and native HTTPS/TLS support via `--ssl-cert` and `--ssl-key`.
