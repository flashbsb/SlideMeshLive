...
8. **Exportação de Dados, Analytics Avançado e Relatórios Executivos Pós-Evento**:
   - A mesa técnica exporta relatórios completos de engajamento em **CSV estruturado**, **Markdown executivo**, **JSON consolidado**, **Deck completo HTML autônomo** e **Relatório Executivo HTML com gráficos inline SVG e suporte a PDF (@media print)**.
   - Histórico multissessão persistido em disco (`sessions_archive/`) com rotação automática das 50 sessões mais recentes e telemetria completa de tempo de permanência (*dwell time*) por slide, engajamento e ranking de votos/perguntas.
...
16. **Arquitetura de Segurança, Matriz de Governança Multi-Auth & Setup Wizard**:
    - **Matriz de Governança Multi-Auth**: Suporte unificado e configurável aos 3 mecanismos de autenticação (**🔑 PIN Mestre / PIN do Deck**, **👤 Contas Locais RBAC com papéis Administrador/Palestrante** e **🌐 Google Workspace SSO Whitelist**), aplicando blindagem contextual nas 5 interfaces do ecossistema: Mesa Técnica (`/admin/`), Telão de Palco (`/presenter/`), Portal Inicial (`/index.html`), SlideMesh Studio (`/import.html`) e Audiência Mobile (`/audience/`).
    - **Proteção de Credenciais & Gatekeeper Server-Side**: O servidor bloqueia acesso direto ao `config/security.json` (`403 Forbidden`) e redige em tempo real a chave `"pin"` de manifestos públicos, delegando 100% da validação de PINs e senhas ao backend (`POST /api/auth/verify-pin` e `/api/auth/login`).
    - **Assistente de Primeiro Uso (Setup Wizard Web & CLI)**: Quando o sistema é iniciado sem credenciais configuradas, o usuário é orientado via `setup.html` (com seleção de métodos ativos, escolha de escopos de proteção e medidor de senha) ou via CLI interativo `python3 server.py --setup` para provisionar a segurança com gravação atômica (`POST /api/auth/setup`).
    - **Painel de Gestão de Segurança & Matriz na Mesa Técnica**: Modal de controle completo acessível pelo botão **`🔐 Segurança`** no Admin com 4 abas interativas (**🔑 Matriz Multi-Auth & Escopos**, **👥 Usuários Locais & Oradores**, **📱 Audiência Offline** e **🌐 Whitelist Google Workspace**) com regra anti-lockout e sincronização instantânea em tempo de execução via `POST /api/security/config` sem reiniciar o servidor Python.
    - **Hardening de Rede & Proteção Anti-Spoofing**: Bloqueio rigoroso de comandos administrativos em `/api/sync` não autorizados (`403 Forbidden`), sanitização anti-XSS de perguntas/votos, rate-limiting anti-flooding (`429 Too Many Requests`) e suporte a HTTPS nativo com `--ssl-cert` e `--ssl-key`.
